...
public class loginadmin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		admin user = udb.loginadmin(URLDecoder.decode(parser.getString("email"), "UTF-8" ),  hpas.hashpassword(URLDecoder.decode(parser.getString("password"), "UTF-8" )));
		if(user==null) {
			response.getWriter().write(new response(false,"b³êdyny email lub has³o").buildjson());
			return;
		}
		logintoken lt = new logintoken(0, null, String.valueOf(usertypes.admin),LocalDateTime.now().plusHours(2),user.getUserid());
		lt.generatetoken();
