...
public class adminauth implements permissioncheck{
	private userdb udb = new userdb();
	@Override
	public boolean haspermission(String token) {
		logintoken lt = udb.readtoken(token);
		if(lt==null)return false;
		if(Enum.valueOf(usertypes.class, lt.getUsertype())==usertypes.admin&&LocalDateTime.now().isBefore(lt.getExpirydate()))return true;
		return false;
	}
